...
outdir = "/Users/gillianchu/raphael/repos/problin/scripts/res_pairwisedist_hess/"
with open(outdir + "ML_triplets_results.txt",'w') as fout:
    for k in [20,30,40,50,100,200,300,400,500,1000,5000]:
        ...
        for D in S:
            ...
            dr_ab = ML_pairwise_estimate(a,b, use_hess=True)[0][2]
            dr_ac = ML_pairwise_estimate(a,c, use_hess=True)[0][2]
            dr_bc = ML_pairwise_estimate(b,c, use_hess=True)[0][2]
            t_abc = triplet_estimate(dr_ab,dr_ac,dr_bc)
         
            dr_ad = ML_pairwise_estimate(a,d, use_hess=True)[0][2]
            dr_cd = ML_pairwise_estimate(c,d, use_hess=True)[0][2]
